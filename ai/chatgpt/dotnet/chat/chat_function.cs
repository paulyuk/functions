...
namespace AI_Functions
{
    public class chat_function
    {
            ...
            FunctionContext executionContext)
        {
            ...
            try {
                ...
                // trying Azure SDK for OpenAI as alternative:
                var client = new OpenAIClient(new Uri(OPENAI_API_URL), new AzureKeyCredential(OPENAI_API_KEY));
                    ...
                    Prompt =
    {
        "What is Azure OpenAI?",
                ...
                logger.LogInformation($"POST to REST API at: {OPENAI_API_URL}");

                Response<Completions> completionsResponse = client.GetCompletions("myModelDeployment", completionsOptions);
                string choice = completionsResponse.Value.Choices[0].Text;
                logger.LogInformation($"Chatbot: {choice}");

                response = req.CreateResponse(HttpStatusCode.OK);
                await response.WriteAsJsonAsync<string>(choice);
            }   
            catch (Exception ex)
            {
